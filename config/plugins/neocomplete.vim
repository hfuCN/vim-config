...
call deoplete#custom#source('_', 'converters', [ 'converter_remove_paren', 'converter_remove_overlap', 'converter_truncate_abbr', 'converter_truncate_menu', 'converter_auto_delimiter', ])
...
" <CR>: If popup menu visible, expand snippet or close popup with selection,
"       Otherwise, check if within empty pair and use delimitMate.
inoremap <silent><expr><CR> pumvisible() ? (neosnippet#expandable() ? neosnippet#mappings#expand_impl() : deoplete#close_popup()) : (delimitMate#WithinEmptyPair() ? "\<C-R>=delimitMate#ExpandReturn()\<CR>" : "\<CR>")
...
" 3. Otherwise, if preceding chars are whitespace, insert tab char
" 4. Otherwise, start manual autocomplete
imap <silent><expr><Tab> pumvisible() ? "\<Down>" : (neosnippet#jumpable() ? "\<Plug>(neosnippet_jump)" : (<SID>is_whitespace() ? "\<Tab>" : deoplete#manual_complete()))

smap <silent><expr><Tab> pumvisible() ? "\<Down>" : (neosnippet#jumpable() ? "\<Plug>(neosnippet_jump)" : (<SID>is_whitespace() ? "\<Tab>" : deoplete#manual_complete()))
